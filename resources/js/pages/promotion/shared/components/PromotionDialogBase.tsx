...
import type { Product, PromotionContext } from "@/pages/promotion/shared/types";

type Props = {
  products?: Product[];
  permissionPrefix: string; // e.g. promotions_entrer | promotions_sortie
  routePrefix: string; // e.g. promotions-entrer | promotions-sortie
  buttonId: string; // unique id per context
  title?: string;
  description?: string;
};

export default function PromotionDialogBase({
  products = [],
  permissionPrefix,
  routePrefix,
  buttonId,
  title = 'Ajouter une Promotion',
  description = 'Remplissez le formulaire pour ajouter une nouvelle promotion',
}: Props) {
  ...
  if (!can(`${permissionPrefix}.create`)) {
    return null;
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button id={buttonId} className="h-10 w-full sm:w-auto">
          {title}
        </Button>
      </DialogTrigger>
      <DialogContent
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                {products.map((product) => (
                  <option
                    ...
                    disabled={product.product_isActive === false}
                    className={product.product_isActive === false ? 'text-gray-400' : ''}
                  >
                    {product.product_Ref} - {product.product_libelle} {product.product_isActive === false && '(inactif)'}
                  </option>
                ))}
              </select>
              {errors.produit_promotionnel_id && (
                <p className="text-xs text-red-500">{errors.produit_promotionnel_id as string}</p>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              {errors.quantite_produit_promotionnel && (
                <p className="text-xs text-red-500">{errors.quantite_produit_promotionnel as string}</p>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              {errors.produit_offert_id && (
                <p className="text-xs text-red-500">{errors.produit_offert_id as string}</p>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              {errors.quantite_produit_offert && (
                <p className="text-xs text-red-500">{errors.quantite_produit_offert as string}</p>
              )}
            </div>
